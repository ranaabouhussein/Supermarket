...
//        val plus : Button=findViewById(R.id.Plus)
        ...
        val delFee:TextView=findViewById(R.id.deliveryFee)
...
//        quantity= arrayOf(0)
        ...
        checkout.setOnClickListener {
            firebaseRef.getReference("Cart").child(Firebase.auth.currentUser?.uid.toString())
                .removeValue()
            firebaseRef.getReference("Cart").child("delivery")
                .removeValue()
...
//                    var TotPrice=firebaseRef.getReference("Cart").child(Firebase.auth.currentUser?.uid.toString()).child("delivery")
                    for (childSnapshot in snapshot.children) {

                        var newPrice = childSnapshot.child("price").value.toString()
                        TotPrice = newPrice.toDouble() + TotPrice + delPrice.toDouble()
            ...
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    for (childSnapshot in snapshot.children) {
                       ...
                       delPrice = childSnapshot.value.toString().toDouble()

//                        delFee.setText(newPrice.toString())

                    }
                    delFee.setText(delPrice.toString())
                    TotalPrice.setText((TotPrice + delPrice).toString())
        ...
        Toast.makeText(this,"Dakhltttttt", Toast.LENGTH_SHORT).show()
